 ...
 * -----------------------------------------
 * Confirms that the shell has values (presumably accurate ones) for each of the
 * enviroment variables listed in the kRequiredEnvironmentVariables array defined
 * below.  The "USER" environment variable is needed to compute the default server
 * port, the "HOST" environment variables is needed, because it needs to be shared
 * with the worker hosts so they know what machine is running the server, and "PWD"
 * is needed to help support relative file paths.
 */
static const char *kRequiredEnvironmentVariables[] = {"HOST", "USER", "PWD"};
void MapReduceServer::ensureEnvironmentVariablesPresent() throw (MapReduceServerException) {
  for (const char *requiredEnvironmentVariable: kRequiredEnvironmentVariables) {
    if (getenv(requiredEnvironmentVariable) == NULL) {
      ostringstream oss;
      oss << requiredEnvironmentVariable << " environment variable must be set.";
...
static const string kConfigFileKeys[] = {
  ...
  "reduce-input-path", "output-path", "log-path",
};
...
void MapReduceServer::applyToServer(const string& key, const string& value) throw (MapReduceServerException) {
  if (key == "mapper") {
    ...
    logPath = ensureDirectoryExists(key, value);
  }
}
 ...
 */
static const string kWorkerNodes[] = {
  "myth2", "myth4", "myth6", "myth8", "myth10", "myth12", "myth16",
  "myth20", "myth22", "myth24", "myth26", "myth30", "myth32"
}; // We're confining ourselves to even-numbered myth machines, CS107 will use odd-numbered machines
static const size_t kNumWorkerNodes = sizeof(kWorkerNodes)/sizeof(kWorkerNodes[0]);
void MapReduceServer::buildIPAddressMap() throw() {
  for (const string& workerNode: kWorkerNodes) {
    struct hostent *he = gethostbyname(workerNode.c_str());
    string ipAddress = inet_ntoa(*(struct in_addr *)(he->h_addr_list[0]));
    ipAddressMap[ipAddress] = workerNode + ".stanford.edu";
  }
}
...
void MapReduceServer::handleRequest(int clientSocket, const string& clientIPAddress) throw() {
  ...
  cout << oslock << "Conversation with " << ipAddressMap[clientIPAddress] << " complete." << endl << osunlock;
}
...
void MapReduceServer::spawnMappers() throw() {
  vector<string> workerNodes(kWorkerNodes, kWorkerNodes + kNumWorkerNodes);
  random_shuffle(workerNodes.begin(), workerNodes.end());
  string command = buildCommand(workerNodes[0], executablePath, mapperExecutable, mapOutputPath, logPath);
  spawnWorker(command);
}
...
string MapReduceServer::buildCommand(const string& workerHost, const string& executablePath, 
                                     const string& executable, const string& outputPath, 
                                     const string& logPath) throw() {
  string userName = getenv("USER");
  string serverHost = getenv("HOST");
  ostringstream oss;
  oss << "ssh " << userName << "@" << workerHost
 ...
 * Method: spawnWorker
 * -------------------
 * Assumes the incoming command is of the form 'ssh poohbear@myth8.stanford.edu '/usr/class/cs110/<other-directories>/mrw <arg1> ... <arg6>',
...
void MapReduceServer::spawnWorker(const string& command) throw() {
  cout << oslock << "Spawning worker with ssh command: " << endl 
       << "\t\"" << command << "\"" << endl << osunlock;
  int status = system(command.c_str());
  cout << oslock << "Remote ssh command executed and return a status " << status << "." << endl << osunlock;
}
