 ...
 * reads a text of key/value-vector pairs via argv[1], outputs
 * a file whose name is supplied via arg[2], and published logging
 * and processing information to the file whose name is supplied
 * via argv[3].
 *
...
int main(int argc, char *argv[]) {
  ...
  logfile << "Processing input chunk: \"" << infileName << "\"." << endl;
  ifstream infile(infileName);
  const char *outfileName = argv[2];
  ofstream outfile(outfileName, ios_base::out | ios_base::trunc);
  aggregateAndPublishWordCount(infile, outfile);
  int result = 
    randomChance(kChanceOfReduceFailure) ? kReduceFailure : kReduceSuccess;
  logfile << "Reducer exit code: " << result << "." << endl;
