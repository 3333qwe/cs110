 ...
 * via argv[1], outputs a file whose name is supplied via
 * argv[2], and publishes logging and processing information
 * the the file whose name is supplied via argv[3].
 *
...
int main(int argc, char *argv[]) {
  ...
  logfile << "Processing infile: \"" << infileName << "\"." << endl;
  ifstream infile(infileName);
  const char *outfileName = argv[2];
  ofstream outfile(outfileName, ios_base::out | ios_base::trunc);
  publishWordCountPairs(infile, outfile);
  int result = 
    randomChance(kChanceOfMapFailure) ? kMapFailure : kMapSuccess;
  logfile << "Mapper exit code: " << result << "." << endl;
